...
vpc <- function(object) {
  ...
  vc <- as.data.frame(VarCorr(object$fit))
  ...
  if (identical(fam, "binomial")) {
    ...
    # residual variance for Gaussian models
...
pcv <- function(null_obj, model_obj) {
  ...
  vc0 <- as.data.frame(VarCorr(null_obj$fit)); v0 <- vc0$vcov[vc0$grp == null_obj$strata][1]
  vc1 <- as.data.frame(VarCorr(model_obj$fit)); v1 <- vc1$vcov[vc1$grp == model_obj$strata][1]
  (v0 - v1) / v0
}
